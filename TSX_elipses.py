...
def produce_midpoint_cell_numbers(mesh):
    ...
    for i, (x, y, _) in enumerate(midpoints):
        if x**2/INNER_X_HALF_AXIS**2 + y**2/INNER_Y_HALF_AXIS**2 <= 1:
            ...
            massif.append(i)
    return inner, outer, massif


def tsx(mesh, lmbda, mu, alpha, cpp, k_massif, k_outer_ellipse, k_inner_ellipse, tau, t_steps_num,
    ...
    in1, in2, massif = produce_midpoint_cell_numbers(mesh)
    k_func.x.array[in1] = k_inner_ellipse
    k_func.x.array[in2] = k_outer_ellipse
    k_func.x.array[massif] = k_massif
    cpp_func.x.array[in1] = cpp_f/100
    cpp_func.x.array[in2] = cpp_f/10
...
if __name__ == '__main__':
    ...
    data2 = tsx(mesh, lmbda, mu, alpha, cpp, 6e-19, 6e-16, 6e-13, timestep,
               t_steps_num=800)
    data2_fp = np.zeros((4, len(data)))
    for i, item in enumerate(data):
    ...
    for i, timeline in enumerate(data2_fp):
        plt.plot(timeline, linestyle='--', color=colors[i])
