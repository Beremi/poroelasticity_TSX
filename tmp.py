...
def produce_midpoint_cell_numbers(mesh):
    ...
    for i, (x, y, _) in enumerate(midpoints):
        if x**2/INNER_X_HALF_AXIS**2 + y**2/INNER_Y_HALF_AXIS**2 <= 1:
            inner.append(i)
        elif if x**2/INNER_Y_HALF_AXIS**2 + y**2/INNER_X_HALF_AXIS**2 <= 1: 
            inner2.append(i)
...
def tsx(mesh, lmbda, mu, alpha, cpp,cpp_f_inner,cpp_f_inner2,cpp_f_outer, k_massif, k_outer_ellipse, k_inner1_ellipse, k_inner2_ellipse, tau, t_steps_num,
